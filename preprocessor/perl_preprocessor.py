...
class PerlPreprocessor:
    ...
    # Maximum depth of nested `include expansion.
    #
    # Include expansion recurses by constructing a new PerlPreprocessor per
    # `include, with no cycle detection and no depth limit. Two files that include
    # each other -- or a file that includes itself -- recursed until CPython's own
    # recursion limit was hit, surfacing as a RecursionError traceback from deep
    # inside the tokenizer rather than a diagnostic naming the offending file, and
    # in a build that raised the limit, as a native stack overflow. A cycle is
    # always a mistake in the source, so a depth budget both terminates it and
    # points at where it happened. 64 is far above any legitimate register-map
    # include nesting.
    MAX_INCLUDE_DEPTH = 64

    def __init__(self, env: 'RDLEnvironment', path: str, search_paths: List[str], incl_ref: Optional[segment_map.IncludeRef]=None, incl_depth: int=0):
        self.env = env
        self.path = path
        self.search_paths = search_paths
        self.incl_ref = incl_ref
        self.incl_depth = incl_depth
    ...
    def get_perl_segments(self, tokens: List[Tuple[str, int, int]]) -> Tuple[List['PPPSegment'], bool]:
        ...
        for typ, start, end in tokens:
            ...
            if typ == "incl":
                ...
                # Enforce the include depth budget before recursing. See
                # MAX_INCLUDE_DEPTH above.
                if self.incl_depth + 1 > self.MAX_INCLUDE_DEPTH:
                    self.env.msg.fatal(
                        "Exceeded maximum `include depth of %d while including '%s'. "
                        "This usually means the include graph contains a cycle."
                        % (self.MAX_INCLUDE_DEPTH, incl_path),
                        DirectSourceRef(self.path, start, end)
                    )
                ...
                incl_file_pp = PerlPreprocessor(
                    self.env, incl_path, self.search_paths, incl_ref,
                    self.incl_depth + 1
                )
