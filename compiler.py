...
class RDLCompiler:
    ...
    def __init__(self, **kwargs: Any):
        ...
        perl_safe_opcodes: List[str]
            ...
            `Perl Safe <https://perldoc.perl.org/Safe.html>`_ compartment.
            Note that a Safe compartment restricts which Perl opcodes may run;
            it is a coarse mitigation and not a security boundary. Do not treat
            it as making an untrusted ``.rdl`` file safe to compile.
            ...
            ``:load``, ``:filesys_read`` and ``:sys_db`` are deliberately absent.
            ``:load`` permits ``require``, which executes an arbitrary module
            outside the compartment and thereby defeats it entirely; the other two
            grant filesystem and passwd/group/host database reads. Re-adding any
            of them means accepting arbitrary code execution or host
            reconnaissance from whoever authored the ``.rdl`` being compiled.
...
class RDLEnvironment:
    ...
    def __init__(self, args_dict: Dict[str, Any]):
        ...
        # Default opcode set. ':load', ':filesys_read' and ':sys_db' were
        # previously included and have been removed:
        #
        #   * ':load' permits require/dofile/caller. `require` loads and executes
        #     an arbitrary module *outside* the compartment, which is a complete
        #     escape -- a Safe compartment that permits ':load' provides no
        #     isolation at all, so a hostile .rdl file's perl preprocessor block
        #     could run any code available on the host.
        #   * ':filesys_read' permits stat/lstat/readlink/fileno, letting an
        #     untrusted .rdl probe the filesystem layout of the build machine.
        #   * ':sys_db' permits getpw*/getgr*/gethost*, i.e. reading /etc/passwd
        #     and /etc/group and issuing name-service lookups.
        #
        # None are needed to expand register maps. Projects that genuinely require
        # one can pass the full list back via the perl_safe_opcodes option -- that
        # is what the option exists for -- but it must be an explicit, reviewed
        # decision rather than the default for every caller.
